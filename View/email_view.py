...
def abrir_config_email(parent_window=None):
    ...
    def toggle_password():
        ...
        if password_visible[0]:
            ...
            btn_ojo.config(text="🙈", bg="#e74c3c")
        else:
            entry_pass.config(show="*")
            btn_ojo.config(text="👁️", bg="#3498db")
    
    btn_ojo = tk.Button(frame_pass, text="👁️", width=3, command=toggle_password, 
                       relief="flat", bd=0, bg="#3498db", fg="white", 
                       font=("Arial", 8, "bold"))
...
def mostrar_config_email_en_frame(parent_frame, callback_volver):
    ...
    btn_ojo = tk.Button(pass_frame, text="👁️", width=4, command=toggle_password, 
                       relief="flat", bd=0, bg="#3498db", fg="white", 
                       font=("Arial", 9, "bold"))
    btn_ojo.pack(side="left")
